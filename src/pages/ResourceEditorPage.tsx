  ...
  const handleResourceSubmit = async (nextStatus: ResourceStatus) => {
    ...
    try {
      if (editingResourceId) {
        ...
        notify({ message: t("teacher.resourceSaveSuccessUpdated"), severity: "success" });
      } else {
        await createTeacherResource(token, payload);
        notify({
          message:
            nextStatus === "DRAFT"
              ? t("teacher.resourceSaveSuccessDraft")
              : t("teacher.resourceSaveSuccessPublished"),
          severity: "success",
        });
      }
      ...
      console.error(err);
      notify({ message: t("teacher.resourceSaveError"), severity: "error" });
    } finally {
